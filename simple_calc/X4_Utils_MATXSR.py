...
def EditMatrix(file):
    ...
    for line in cf:
        ...
        if line.find('8d') != -1 or last_line[0] == '8d':
            ...
            if transfer_initialize == False:
                transfer_mat = np.zeros([n_groups,n_groups,Pn_order])
            ...
            # xs_array.append(np.zeros([n_groups,n_groups]))
            # xs_array.append(np.zeros([n_groups,n_groups]))
            xs_array = np.zeros([n_groups, n_groups, Pn_order])
            ...
            for arr_grp in range(len(info_grp_end)):                        # departing group
                for p in range(Pn_order):                                        # legendre coefficient number
                    for dep_grp in reversed(range(info_grp_end[arr_grp])): # arrival group
                        ...
                        #if abs(xs_list[grps_done*Pn_order+p*info_grp_end[arr_grp]+dep_grp]) > 1e-18:
            ...
            if loop_name != 'free_mat' and loop_name != 'fe_mat' and loop_name != 'fe$_mat' and loop_name != 'nx_mat':# and loop_name[2] not in numbers or loop_name == 'n01_mat':
                #for p in range(Pn_order):
                transfer_mat += xs_array
    matxsr_data['transfer_mats'] = np.copy(transfer_mat)
    # matxsr_data['transfer_mat_free'] = np.copy(transfer_mat)
    ...
    #         for arr_grp in range(n_groups):
    #           if abs(matxsr_data['free_mat'][dep_grp, arr_grp, p]) > 1e-18:
    #             matxsr_data['transfer_mat_free'][dep_grp, arr_grp, p] = matxsr_data['free_mat'][dep_grp, arr_grp, p]
    ...
    matxsr_data['sigma_t'] = matxsr_data['ntot0']
    matxsr_data['sigma_heat'] = matxsr_data['heat']
