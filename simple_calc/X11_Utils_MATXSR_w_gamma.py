...
def pad_or_truncate(given_list, target_len):
    return given_list[:target_len] + [0]*(target_len - len(given_list))
...
def EditMatrix(file):
    ...
    for line in cf:
        ...
        if line.find('8d') != -1 or last_line[0] == '8d':
            ...
            if info_name+'_mat' in matxsr_data.keys():
                info_name = info_name+'_'+particle_names[int(final_particle[data_type])-1]
            ...
            key_name = particle_names[initial_particle[data_type]-1]+'_to_'+particle_names[final_particle[data_type]-1]
            transfer_keys[key_name][data_type_names[data_type]].append(info_name+'_mat')
    ...
    max_num_moms = 0
    for num_transfer, transfer in enumerate((transfer_keys)):
        for num_data, data_type in enumerate(transfer_keys[transfer]):
            for num_rxn, rxn in enumerate(transfer_keys[transfer][data_type]):
                if np.shape(matxsr_data[rxn])[2] > max_num_moms:
                    max_num_moms = np.shape(matxsr_data[rxn])[2]
    ...
    for nrg in range(particle_types):    
        ...
        for grp in range(len(matxsr_data[particle_names[nrg]+'_energy_bins'])-1):
            ...
            if particle_names[nrg] == 'n':
                matxsr_data['neutron_gs'].append([float(grp), float(lo_bound), float(hi_bound)])
            elif particle_names[nrg] == 'g':
                matxsr_data['gamma_gs'].append([float(grp), float(lo_bound), float(hi_bound)])
            
    n_groups = len(matxsr_data['neutron_gs'])
    g_groups =  len(matxsr_data['gamma_gs'])        
    G = len(matxsr_data['neutron_gs']) + len(matxsr_data['gamma_gs'])
    transfer_mat = []
    
    def AddNeutronTransfer(key):
        transfers = np.copy(matxsr_data[key])
        n_mom = np.shape(transfers)[2]
        # print(key)
        for moment in range(n_mom):
            transfer_mat[moment][0:(n_groups),0:(n_groups)] += transfers[:,:,moment]
            
    def AddGammaTransfer(key):
        transfers = np.copy(matxsr_data[key])
        n_mom = np.shape(transfers)[2]
        for moment in range(n_mom):
            transfer_mat[moment][n_groups:(G),n_groups:(G)] += transfers[:,:,moment]
    
    def AddNtoGTransfer(key):
        transfers = np.copy(matxsr_data[key])
        n_mom = np.shape(transfers)[2]
        for moment in range(n_mom):
            transfer_mat[moment][0:n_groups,n_groups:(G)] += transfers[:,:,moment]
        
    def AddFreeTransfer(key):
        transfers = np.copy(matxsr_data[key])
        n_mom = np.shape(transfers)[2]
        for moment in range(n_mom):
            for dep_grp in range(n_groups):
                for arr_grp in range(n_groups):
                    if transfers[dep_grp,arr_grp,moment] > 1e-18:
                        transfer_mat[moment][dep_grp,arr_grp] = transfers[dep_grp,arr_grp,moment]
    
    def AddSABTransfer(keys):
        transfers = np.copy(matxsr_data[keys[0]])
        for scat_mat in range(1,len(keys)):
            transfers += matxsr_data[keys[scat_mat]]
        n_mom = np.shape(transfers)[2]
        for moment in range(n_mom):
            for dep_grp in range(n_groups):
                for arr_grp in range(n_groups):
    ...
    for m in range(0, max_num_moms):
        transfer_mat.append(np.zeros([G, G]))
    for num_transfer, transfer in enumerate((transfer_keys)):
        for num_data, data_type in enumerate(transfer_keys[transfer]):
            for num_rxn, rxn in enumerate(transfer_keys[transfer][data_type]):
                if transfer == 'n_to_n' and data_type == 'nscat':
                    AddNeutronTransfer(rxn)
                if transfer == 'g_to_g' and data_type == 'gscat':
                    AddGammaTransfer(rxn)
                if transfer == 'n_to_g' and data_type == 'ng':
                    AddNtoGTransfer(rxn)
    
                    ############## need to see if both free mat and s alpha beta are done
    if 'free_mat' in matxsr_data.keys():
        AddFreeTransfer('free_mat')
        
    if 'n_to_n' in transfer_keys.keys():
        if 'ntherm' in transfer_keys['n_to_n'].keys():
            if len(transfer_keys['n_to_n']['ntherm']) > 1:
                ntherm_list = transfer_keys['n_to_n']['ntherm']
                if 'free_mat' in ntherm_list:
                    ntherm_list.remove('free_mat')
                AddSABTransfer(ntherm_list)
    ...
    matxsr_data['transfer_matrices'] = np.copy(transfer_mat)
    ...
    if 'n' in particle_names and 'g' in particle_names:
        matxsr_data['sigma_t']    = np.append(matxsr_data['ntot0'],matxsr_data['gtot0'])    # adjust naming for total xs
    elif 'n' in particle_names and 'g' not in particle_names:
        matxsr_data['sigma_t']    = matxsr_data['ntot0']   # adjust naming for total xs
    elif 'n' not in particle_names and 'g' in particle_names:
        matxsr_data['sigma_t']    = matxsr_data['gtot0']   # adjust naming for total xs
    
    # matxsr_data['sigma_s']   = np.sum(matxsr_data['transfer_matrices'][0], axis = 1)
    matxsr_data['sigma_s'] = np.zeros(G)
    if n_groups > 0:
        for grp in range(n_groups):
            matxsr_data['sigma_s'][grp]    = np.sum(matxsr_data['transfer_matrices'][0][grp,:n_groups])
    if g_groups > 0:
        if (len(matxsr_data['gcoh']) > 0) and (len(matxsr_data['ginch']) > 0):
            for i in range (len(matxsr_data['gcoh'])):
                matxsr_data['sigma_s'][n_groups + i] = matxsr_data['gcoh'][i] + matxsr_data['ginch'][i]
        
        
    if 'nelas' in matxsr_data.keys() and 'ninel' in matxsr_data.keys():
        if len(matxsr_data['nelas']) < len(matxsr_data['sigma_t']):
            matxsr_data['sigma_a'] = np.copy(np.array(matxsr_data['sigma_t']))
            matxsr_data['sigma_a'][:len(matxsr_data['nelas'])] -= (np.array(matxsr_data['nelas']) + np.array(matxsr_data['ninel']))
        else:
        ...
        if len(matxsr_data['nelas']) < len(matxsr_data['sigma_t']):
            matxsr_data['sigma_a'] = np.copy(np.array(matxsr_data['sigma_t']))
            matxsr_data['sigma_a'][:len(matxsr_data['nelas'])] -= (np.array(matxsr_data['nelas']))
        else:
        ...
        if len(matxsr_data['ninel']) < len(matxsr_data['sigma_t']):
            matxsr_data['sigma_a'] = np.copy(np.array(matxsr_data['sigma_t']))
            matxsr_data['sigma_a'][:len(matxsr_data['ninel'])] -= np.array(matxsr_data['ninel'])
        else:
            matxsr_data['sigma_a']    =np.array(matxsr_data['sigma_t']) - np.array(matxsr_data['ninel'])
        
    if g_groups > 0:
        if "gpair" in matxsr_data.keys():
            sig_pairprod = np.copy(matxsr_data['gpair'])
        # if "gpair" in matxsr_data.keys():
        #     data = matxsr_data['gpair']
        #     for entry in data:
        #         g = entry[0]
        #         v = entry[1]
        #         sig_pairprod[g_groups - g - 1] += v
        
        for grp in range (len(sig_pairprod)):
            matxsr_data['sigma_a'][n_groups + grp] = matxsr_data['sigma_t'][n_groups + grp] - matxsr_data['sigma_s'][n_groups + grp] - sig_pairprod[grp]    
    
    if g_groups > 0:
        if 'gheat' in matxsr_data.keys():
            matxsr_data['sigma_heat'] = np.append(matxsr_data['heat'],matxsr_data['gheat'])
        else:
            matxsr_data['sigma_heat'] = pad_or_truncate(matxsr_data['heat'], G)
    else:
        matxsr_data['sigma_heat'] = matxsr_data['heat']     # adjust naming for heating xs
    
    matxsr_data['inv_velocity']    = pad_or_truncate(matxsr_data['invel'], G)
    
    
    # matxsr_data['sigma_s']    = np.sum(matxsr_data['transfer_matrices'][0], axis = 1)
    #matxsr_data['sigma_t']    = matxsr_data['sigma_s']+matxsr_data['sigma_t']    # adjust naming for total xs
    #matxsr_data['sigma_t']    = matxsr_data['ntot0']    # adjust naming for total xs
    
    if 'nftot' in matxsr_data.keys():
        matxsr_data["sigma_f"] = np.copy(matxsr_data["nftot"])
        matxsr_data['sigma_f'] = pad_or_truncate(matxsr_data['sigma_f'], G)
    
    ######## I don't actually know how MATXSR calssifies these labels
    if 'totnu' in matxsr_data.keys():
        matxsr_data["nu_total"] = np.copy(matxsr_data["totnu"])
        matxsr_data['nu_total'] = pad_or_truncate(matxsr_data['nu_total'], G)
    if 'promptnu' in matxsr_data.keys():
        matxsr_data["nu_prompt"] = np.copy(matxsr_data["promptnu"])
        matxsr_data['nu_prompt'] = pad_or_truncate(matxsr_data['nu_prompt'], G)
    if 'chiprompt' in matxsr_data.keys():
        matxsr_data["chi_prompt"] = np.copy(matxsr_data["chiprompt"])
        matxsr_data['chi_prompt'] = pad_or_truncate(matxsr_data['chi_prompt'], G)
        
    if 'delaynu' in matxsr_data.keys():
        matxsr_data["nu_delayed"] = np.copy(matxsr_data["delaynu"])
        matxsr_data['nu_delayed'] = pad_or_truncate(matxsr_data['nu_delayed'], G)
    if 'decayconst' in matxsr_data.keys():
        matxsr_data["decay_constants"] = np.copy(matxsr_data["decayconst"])
        matxsr_data['decay_constants'] = pad_or_truncate(matxsr_data['decay_constants'], G)
    J = len(matxsr_data['decay_constants'])
    
    if 'chidelay' in matxsr_data.keys():
        matxsr_data["chi_delayed"] = np.copy(matxsr_data["chidelay"])
        matxsr_data['chi_delayed'] = pad_or_truncate(matxsr_data['chi_delayed'], G)
    

    
    matxsr_data['gamma'] = np.zeros(J)
    if np.sum(matxsr_data['nu_delayed']) > 0 and np.sum(matxsr_data['chi_delayed']) > 0:
        matxsr_data['gamma'] = np.sum(matxsr_data['chi_delayed'], axis = 0)
    matxsr_data['chi_delayed'] /= matxsr_data['gamma']
...
matxsr_data2, problem_description = EditMatrix('../5_9 meeting/NJOY_files/tape41_graphite')
